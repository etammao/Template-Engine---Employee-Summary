...
async function main(){
    ...
    const managerData = await inquirer.prompt([
        {name: 'name', type:'input', message:`What's your manager's name?`},
        {name: 'email', type:'input', message:`What's your manager's email?`},
        {name: 'officeNumber', type:'input', message:`What's your their officeNumber?`},
    ...
    for (let userCnt=1; userCnt<=managerData.count;userCnt++){
        ...
        const user=await inquirer.prompt([
            {name:'type',type:'list',message: `For person ${userCnt}/${managerData.count}, are they an:`, choices:['intern','engineer']}
        ])
        ...
        if(user.type=='engineer'){
            const userData= await inquirer.prompt([
                ...
                {name:'github', type:'input',message:`What is the engineer's github?`},
            ]);
